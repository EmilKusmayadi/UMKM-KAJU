                    ...
                    <tr>
                        ...
                        <td class="py-2 px-4 border-b">
                            @if ($datas->about_image)
                                <img style="max-width: 50px; max-height:50px" src="{{ url('about_image').'/'.$datas->about_image }}" alt="">
                            @endif
                        </td>
                        <td class="py-2 px-4 border-b flex">
                            <a href="{{ url('/about/'.$datas->id.'/edit') }}" class="text-gray-100 font-semibold bg-yellow-500 p-2 rounded-lg me-5">Edit</a>
                            <form onsubmit="return confirm('Yakin mau hapus data')" action="{{ '/about/'.$datas->id }}" method="POST" class="text-gray-100 inline font-semibold bg-red-500 p-2 rounded-lg">
                                @csrf
                                @method('DELETE')
