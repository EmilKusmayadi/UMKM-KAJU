<x-app-layout>
    ...
    <form action="{{ '/banners/' . $data->id }}" method="POST" enctype="multipart/form-data"
        ...
        <div class="mb-4">
            ...
            <input type="text" name="banner_title" id="banner_title" class="mt-1 p-2 w-full border rounded-md"
                value="{{ $data->banner_title }}">
        </div>
        @if ($data->banner_image)
            <div class="mb-3">
                <img style="max-width: 50px; max-height:50px;" src="{{ url('banner_image') . '/' . $data->banner_image }}"
                    alt="">
            </div>
        @endif
        <div class="mb-4">
            <label for="banner_image" class="block text-sm font-medium text-gray-600">banner_image</label>
            <input type="file" name="banner_image" id="banner_image" class="mt-1 p-2 w-full border rounded-md"
                value="{{ $data->banner_image }}">
        </div>
        <div class="mb-4">
            <label for="banner_url" class="block text-sm font-medium text-gray-600">banner_url</label>
            <input type="text" name="banner_url" id="banner_url" class="mt-1 p-2 w-full border rounded-md"
