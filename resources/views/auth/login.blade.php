    ...
    <div class="w-full h-full px-3 lg:pl-8 flex flex-col justify-center items-center lg:items-start z-50">
        ...
        <div class="flex flex-col gap-y-10">
            <form method="POST" action="{{ route('login') }}" class="flex flex-col w-full lg:w-80 mt-12">
                ...
                <input type="email" id="Email" name="email" :value="old('email')" required autofocus autocomplete="username" class="bg-transparent outline-none border-b border-transparent border-b-black">
                <x-input-error :messages="$errors->get('email')" class="mt-2" />
                <label for="Password" class="mt-5">Password</label>
                <input type="password" name="password"
                required autocomplete="current-password" id="Password" class="bg-transparent outline-none border-b border-transparent border-b-black">
            ...
            <div class="flex flex-col gap-y-5">
                <div class="flex gap-x-4">
                    ...
                    <button class="border border-[#E8C597] w-36 font-thin text-sm py-1 rounded-md">Forgot Password</button>
                </div>
